...
function DayBlock({ dateKey, screenScale, isToday, events, rotationSeconds }) {
  return (
    <div className={`relative z-0 flex min-h-0 min-w-0 flex-col items-center justify-center rounded-2xl border-4 p-3 text-center ${events.length > 0 ? 'z-40' : ''} ${isToday ? 'border-amber-500 bg-amber-50' : 'border-slate-400 bg-white'}`}>
      <div className="font-black leading-none text-emerald-900" style={{ fontSize: `${70 * screenScale}px` }}>{formatHebrewDay(dateKey)}</div>
      <div className="mt-2 font-black text-slate-700" style={{ fontSize: `${30 * screenScale}px` }}>{new Date(`${dateKey}T12:00:00Z`).getUTCDate()}</div>
      <div className="mt-1 font-bold text-slate-600" style={{ fontSize: `${24 * screenScale}px` }}>{formatWeekday(dateKey)}</div>
      <div className="mt-1 font-semibold text-slate-500" style={{ fontSize: `${20 * screenScale}px` }}>{formatHebrewMonthYear(dateKey)}</div>
...
function EventDetailsCard({ event, screenScale }) {
  return (
    <div className="min-w-0 rounded-xl border-4 bg-white px-6 py-4 shadow-sm" style={{ borderColor: event.color || '#5FAFA8' }}>
      ...
      <div className="mt-2 font-black text-slate-800" style={{ fontSize: `${32 * screenScale}px` }}>{formatHebrewDate(event.eventDate)}</div>
      <div className="font-bold text-slate-600" style={{ fontSize: `${27 * screenScale}px` }}>{formatEventTime(event)}{event.location ? ` · ${event.location}` : ''}</div>
  ...
  return (
    <div className="shrink-0 rounded-2xl border-4 border-slate-400 bg-slate-200 p-4 shadow-inner">
      <div className="mb-3 flex items-center justify-between gap-3">
        <div className="font-black text-slate-900" style={{ fontSize: `${34 * screenScale}px` }}>פרטי אירועים</div>
...
export default function CalendarDisplay({ events = [], screenScale = 1, rotationSeconds = 6, upcomingEvent }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.98 }} className="absolute inset-0 z-40 flex min-w-0 flex-col overflow-hidden rounded-3xl bg-slate-100 p-4" dir="rtl">
      ...
      <div className="mb-2 grid shrink-0 grid-cols-7 gap-2">{DAY_HEADERS.map(day => <div key={day} className="rounded-lg bg-slate-800 py-1 text-center font-black text-white" style={{ fontSize: `${17 * displayScale}px` }}>{day}</div>)}</div>
